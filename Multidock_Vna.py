...
def filtro_log(pdb=False):
        ...
        newpy.write('\t\t\t\tbreak\n')
        newpy.write('\t\tfinal_out = out_file\n')
        if pdb:
        ...
        newpy.write('\t\t\tcmd("mv %s Ok_dock"%(final_out))\n')
        newpy.write('\t\t\tcmd("mv %s Ok_dock"%(arg[1]))\n')
        newpy.write('\t\telse:\n') # -4.5 kcal/mol > affinity  :: good docking
        newpy.write('\t\t\tcmd("mv %s Nice_dock"%(final_out))\n')
...
def multi_inp(cent_x = -18.287, cent_y = -9.034, cent_z = 7.361, max_dv = 5,
                          ...
                          box_x = 15, box_y = 15, box_z = 15, exh = 10, multi= 10,
...
if __name__ == "__main__":
        ...
        while i < len(arg):
                if inst_only:
                        ...
                        print("\t-pdbqt2pdb\t\t(If openBabel is installed) Converts the PDBQT format of vina output to PDB with obabel.\n")
        ...
        if not inst_only:
                if '' not in [recpt, ligand]:
                        multi_inp(cent_x=cent_x,cent_y=cent_y,cent_z=cent_z,multi=multi,max_dv=max_dv,recpt=recpt, ligand=ligand, out_na=out_na,box_x=box_x,box_y=box_y,box_z=box_z,exh=exh,vina_home=vina_home)
                        filtro_log(pdb=convert)
                        cmd('sh exec_vina.sh')
                else:
